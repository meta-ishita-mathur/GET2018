...
public class BSTDictionary implements Dictionary
{
	...
	{        
		ArrayList<DictionaryEntry> sortedEntryList = new ArrayList<DictionaryEntry>();
	...
	{
		ArrayList<DictionaryEntry> sortedSubList, sortedEntryList;
		if(firstKey == null || secondKey == null)
			throw new AssertionError("invalid input");

		sortedSubList = new ArrayList<DictionaryEntry>();
		sortedEntryList = getSortedList();
		...
		while(iterator.hasNext())
		{
			...
			if(firstKey.compareTo(entry.getKey()) < 0 && secondKey.compareTo(entry.getKey()) > 0)
	 ...
	 */
	private void inorderTraversal(BSTNode root, ArrayList<DictionaryEntry> sortedEntryList)
	{
		if (root != null)
		{
